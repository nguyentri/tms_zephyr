...
// Register a logging module for this file (CM7 main)
 ...
 * - rust_openamp_init_cm7: Initialize Rust-side OpenAMP state
 * - rust_openamp_send_message_cm7: Log and count messages sent from Rust
 * - rust_openamp_process_received_cm7: Process incoming messages in Rust
 */
extern void rust_openamp_init_cm7(void);
extern int rust_openamp_send_message_cm7(const char* message);
extern void rust_openamp_process_received_cm7(const char* data, size_t len);
...
{
    LOG_INF("CM7: Received message: %.*s", (int)len, (char *)data);

    // Forward the received message to Rust for further processing
    rust_openamp_process_received_cm7((const char*)data, len);
}
...
int main(void)
{
     ...
     * - Log and count it in Rust (rust_openamp_send_message_cm7)
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ret = rust_openamp_send_message_cm7(msg);
        if (ret < 0) {
    ...
    LOG_INF("CM7 communication test completed");
    return 0;
}
