...
// Global state for OpenAMP CM7
static mut INITIALIZED: bool = false;
static mut MESSAGE_COUNT: u32 = 0;

#[no_mangle]
pub extern "C" fn rust_openamp_init_cm7() {
    unsafe {
        if !INITIALIZED {
...
#[no_mangle]
pub extern "C" fn rust_openamp_send_message_cm7(message: *const c_char) -> c_int {
    unsafe {
        if message.is_null() {
            return -1;
        }
        ...
        let log_msg = b"Rust CM7: Preparing to send message #%d (len=%d)\n\0";
...
#[no_mangle]
pub extern "C" fn rust_openamp_process_received_cm7(data: *const c_char, len: usize) {
    unsafe {
        if data.is_null() || len == 0 {
            return;
        }

        let log_msg = b"Rust CM7: Processing received message (len=%d)\n\0";
...
#[no_mangle]
pub extern "C" fn rust_openamp_get_stats_cm7() -> u32 {
    unsafe {
        MESSAGE_COUNT
    }
}
...
fn panic(_info: &core::panic::PanicInfo) -> ! {
    unsafe {
        let msg = b"Rust CM7 panic occurred!\n\0";
        printk(msg.as_ptr() as *const c_char);
    }
    loop {}
}
